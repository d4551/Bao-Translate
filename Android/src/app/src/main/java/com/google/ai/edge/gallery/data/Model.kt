...
enum class ModelCapability {
  ...
  @SerialName("speculative_decoding") SPECULATIVE_DECODING,
}
...
@Serializable data class ModelFile(val fileName: String, val commitHash: String)
...
data class Model(
  ...
  // Runtime model instance. Type intentionally erased: each helper (LlmModelHelper,
  ...
  // Lifecycle follow-up: replace the current init/cleanup flags with a per-model operation queue
  // when multiple runtime helpers need ordered cancellation or restart semantics.
  ...
  fun getIntConfigValue(key: ConfigKey, defaultValue: Int = 0): Int {
    val cv = getTypedConfigValue(key, ValueType.INT, ConfigValue.IntValue(defaultValue))
    return (cv as? ConfigValue.IntValue)?.value ?: defaultValue
  }

  fun getFloatConfigValue(key: ConfigKey, defaultValue: Float = 0.0f): Float {
    val cv = getTypedConfigValue(key, ValueType.FLOAT, ConfigValue.FloatValue(defaultValue))
    return (cv as? ConfigValue.FloatValue)?.value ?: defaultValue
  }

  fun getBooleanConfigValue(key: ConfigKey, defaultValue: Boolean = false): Boolean {
    val cv = getTypedConfigValue(key, ValueType.BOOLEAN, ConfigValue.BooleanValue(defaultValue))
    return (cv as? ConfigValue.BooleanValue)?.value ?: defaultValue
  }

  fun getStringConfigValue(key: ConfigKey, defaultValue: String = ""): String {
    val cv = getTypedConfigValue(key, ValueType.STRING, ConfigValue.StringValue(defaultValue))
    return (cv as? ConfigValue.StringValue)?.value ?: defaultValue
  }
  ...
  private fun getTypedConfigValue(
    ...
    val raw = configValues[key.label]
    return if (raw == null) defaultValue else ConfigValue.from(raw, valueType)
  }
}
