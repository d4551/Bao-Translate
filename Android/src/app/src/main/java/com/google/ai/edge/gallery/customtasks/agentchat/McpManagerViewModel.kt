...
constructor(
      ...
      runCatching {
        ...
        val loadedStates = savedServers.map { serverProto ->
          runCatching {
            ...
            BaoLog.e(TAG, "Error loading MCP server: ${serverProto.url}", e)
        ...
        BaoLog.e(TAG, "Error reading saved MCP servers", e)
  ...
  fun addMcpServer(
    ...
    viewModelScope.launch(Dispatchers.IO) {
      runCatching {
        ...
        BaoLog.d(TAG, "Analytics: mcp_management, action=add_server, status=success")
        ...
        BaoLog.e(TAG, "Error adding MCP server: $url", e)
  ...
  private suspend fun initializeClientAndLoadTools(
    ...
    BaoLog.d(TAG, "Initializing MCP for $url...")
    val client =
      Client(
        clientInfo =
          Implementation(name = "google-ai-edge-gallery", version = ProjectConfig.versionName)
      )
    ...
    BaoLog.d(TAG, "Loaded ${mcpTools.size} tools from $url: ${mcpTools.joinToString { it.name }}")
