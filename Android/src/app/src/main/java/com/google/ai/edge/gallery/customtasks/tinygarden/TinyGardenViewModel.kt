...
constructor(
  ...
  fun getCommand(
    ...
    // Count turn.
    incrementNumTurns()
    BaoLog.d(TAG, "Turn #: ${uiState.value.numTurns}")

    // Add user prompt to history.
    this.addMessage(message = ChatMessageText(content = instructionText, side = ChatSide.USER))

    viewModelScope.launch(Dispatchers.Default) {
      BaoLog.d(TAG, "Start processing user instruction: '$instructionText'")
      ...
      responseMessage
        ...
        .onFailure { e ->
          BaoLog.e(TAG, "Failed to run inference", e)
  ...
  /** Persists that Tiny Garden has been run at least once (fire-and-forget). */
  fun markTinyGardenRun() {
    viewModelScope.launch { dataStoreRepository.setHasRunTinyGarden(true) }
  }
  ...
  fun resetConversation(
    ...
    viewModelScope.launch(Dispatchers.Default) {
      ...
      BaoLog.d(TAG, "Current system prompt:\n$curSystemPrompt")
