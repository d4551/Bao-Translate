...
private suspend fun playAudio(
  ...
  BaoLog.d(TAG, "Start playing audio...")

  // `try/finally` (no `catch`) is the documented resource-cleanup pattern and is allowed under
  // the no-try/catch rule. Errors during playback are intentionally ignored.
  try {
...
private fun stopPlayAudio(audioTrackState: MutableState<AudioTrack?>) {
  BaoLog.d(TAG, "Stopping playing audio...")
