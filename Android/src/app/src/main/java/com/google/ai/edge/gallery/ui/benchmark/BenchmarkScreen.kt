...
fun BenchmarkScreen(
  ...
  val downloadedLlmModelNames =
    remember(modelManagerUiState.tasks, modelManagerUiState.modelDownloadStatus) {
  ...
  var selectedModel by
    remember(selectedModelName) {
      mutableStateOf(modelManagerViewModel.getModelByName(name = selectedModelName) ?: initialModel)
    }
  ...
  val selectedModelAvailableForBenchmark = selectedModelName in downloadedLlmModelNames
      ...
      Box(
        ...
        Column(modifier = Modifier.padding(16.dp).fillMaxSize()) {
          ...
          Column(
            ...
            if (!selectedModelAvailableForBenchmark) {
              Text(
                stringResource(R.string.benchmark_download_required),
                style = MaterialTheme.typography.bodyMedium,
          ...
          Row(
            ...
            Button(
              enabled =
                selectedModelAvailableForBenchmark && sumOfPrefillAndDecodeTokens <= maxToken,
              onClick = {
...
private fun getStringConfigValue(values: Map<String, Any>, key: ConfigKey): String {
  return ConfigValue.from(value = values.get(key.label) ?: "", valueType = ValueType.STRING)
    .let { (it as? ConfigValue.StringValue)?.value ?: "" }
}

private fun getIntConfigValue(values: Map<String, Any>, key: ConfigKey): Int {
  return ConfigValue.from(value = values.get(key.label) ?: 0, valueType = ValueType.INT)
