...
class DefaultDownloadRepository(
  ...
  override fun cancelAll(onComplete: () -> Unit) {
    workManager
      .cancelAllWork()
      .result
      .addListener({ onComplete() }, ContextCompat.getMainExecutor(context))
  }
      ...
      object : androidx.lifecycle.Observer<WorkInfo?> {
        ...
        when (workInfo.state) {
          ...
          WorkInfo.State.SUCCEEDED -> {
            BaoLog.d("repo", "worker %s success".format(workerId.toString()))
        ...
        // Self-remove once the work reaches a terminal state so the observer (and the model/VM
        // references it captures) is not leaked for the process lifetime.
