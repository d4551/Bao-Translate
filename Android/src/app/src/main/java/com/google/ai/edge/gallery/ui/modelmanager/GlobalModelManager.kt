...
fun GlobalModelManager(
    ...
    rememberLauncherForActivityResult(
      ...
      if (result.resultCode == android.app.Activity.RESULT_OK) {
        result.data?.data?.let { uri ->
          ...
          BaoLog.d(TAG, "Selected file: $fileName")
        ...
        } ?: run { BaoLog.d(TAG, "No file selected or URI is null.") }
      } else {
        BaoLog.d(TAG, "File picking cancelled.")
      }
    }
  ...
  val modelVariants by
    remember(uiState.modelImportingUpdateTrigger) {
      derivedStateOf {
        ...
        allModels
          .mapNotNull { model -> model.parentModelName?.let { parent -> parent to model } }
      ...
      LazyColumn(
        ...
        item(key = "promo") {
          AnimatedVisibility(
            ...
            PromoBannerGm4(
              onDismiss = {
                showPromo = false
                viewModel.markPromoViewed(promoId = promoId)
              }
            )
          }
        }
        ...
        items(builtInModels) { model ->
          ...
          ModelItem(
            ...
            expanded = expanded,
            showBenchmarkButton = model.isLlm && model.runtimeType == RuntimeType.LITERT_LM,
    ...
    ModalBottomSheet(onDismissRequest = { showImportModelSheet = false }, sheetState = sheetState) {
      ...
      Box(
        ...
        Row(
          ...
          Text(
            stringResource(R.string.from_local_model_file),
  ...
  if (showImportingDialog) {
    val modelImportedMessage = stringResource(R.string.model_imported_successfully)
    selectedLocalModelFileUri.value?.let { uri ->
      selectedImportedModelInfo.value?.let { info ->
        ModelImportingDialog(
          ...
          onDone = {
            ...
            scope.launch { snackbarHostState.showSnackbar(modelImportedMessage) }
          },
        )
      }
    }
  ...
  if (showUnsupportedFileTypeDialog) {
    AlertDialog(
      ...
      text = { Text(stringResource(R.string.only_task_or_litertlm_supported)) },
      confirmButton = {
  ...
  if (showUnsupportedWebModelDialog) {
    AlertDialog(
      ...
      text = { Text(stringResource(R.string.web_only_model_not_supported)) },
      confirmButton = {
