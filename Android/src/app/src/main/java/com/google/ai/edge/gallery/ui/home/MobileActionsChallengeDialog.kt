...
fun MobileActionsChallengeDialog(
  ...
  ModalBottomSheet(onDismissRequest = onDismiss, sheetState = sheetState) {
    Column(modifier = Modifier.padding(16.dp)) {
      ...
      // Render the four instruction lines as separate localized Text items.
      // The instruction_1 key embeds an <a href> link in the original HTML; here we render
      // it as plain localized text plus a trackable ClickableLink for the guide URL below.
      val instructionStyle = MaterialTheme.typography.bodyMedium
      Text(
        text = stringResource(R.string.mobile_actions_challenge_instruction_1),
        color = MaterialTheme.colorScheme.onSurface,
        style = instructionStyle,
      )
      Text(
        text = stringResource(R.string.mobile_actions_challenge_instruction_2),
        color = MaterialTheme.colorScheme.onSurface,
        style = instructionStyle,
      )
      Text(
        text = stringResource(R.string.mobile_actions_challenge_instruction_3),
        color = MaterialTheme.colorScheme.onSurface,
        style = instructionStyle,
      )
      Text(
        text = stringResource(R.string.mobile_actions_challenge_instruction_4),
        color = MaterialTheme.colorScheme.onSurface,
        style = instructionStyle,
      )
      ClickableLink(
        url = guideUrl,
        linkText = stringResource(R.string.mobile_actions_challenge_instruction_1_link),
