...
open class AgentTools() : ToolSet {
  ...
  fun loadSkill(
    ...
    return runBlocking(Dispatchers.Default) {
      ...
      BaoLog.d(TAG, "load skill. Skill content:\n$skillContent")
      if (skill != null) {
        _actionChannel.send(
  ...
  fun runMcpTool(
    ...
    return runBlocking(Dispatchers.IO) {
      ...
      if (serverState == null) {
        BaoLog.w(TAG, "MCP server or tool not found for: $toolName")
              ...
              request =
                CallToolRequest(
                  CallToolRequestParams(
                    name = toolName,
                    arguments =
                      kotlinx.serialization.json.Json.parseToJsonElement(input).jsonObject
                  )
                )
            )
      ...
      if (callException != null) {
        BaoLog.e(TAG, "Error calling MCP tool", callException)
        _actionChannel.send(
          SkillProgressAgentAction(
            label = "Error calling MCP tool \"$toolName\"",
            inProgress = false,
            addItemTitle = "Call MCP tool \"$toolName\" failed",
            addItemDescription = callException.message ?: "Unknown error",
          )
        )
        logMcpExecution(success = false, errorType = "exception")
        return@runBlocking mapOf("error" to (callException.message ?: "Unknown error"), "status" to "failed")
      }
      mapOf("error" to "Unexpected MCP tool state", "status" to "failed")
    }
  }
  ...
  fun runJs(
    ...
    return runBlocking(Dispatchers.Default) {
      ...
      if (skill.requireSecret) {
        ...
        if (savedSecret == null || savedSecret.isEmpty()) {
          ...
          if (secret.isNotEmpty()) {
            ...
            BaoLog.d(TAG, "Got Secret from ask info dialog: ${secret.substring(0, 3)}")
          } else {
            BaoLog.d(TAG, "The ask info dialog got cancelled. No secret.")
          }
        } else {
          secret = savedSecret
        }
      ...
      BaoLog.d(TAG, "Calling JS script.\n- url: $url\n- data: $data")
      ...
      val result =
        runCatching { action.result.await() }
          .getOrElse { e ->
            BaoLog.e(TAG, "JS skill execution failed", e)
            return@runBlocking mapOf(
              "error" to (e.message ?: "Skill execution failed"),
              "status" to "failed",
            )
          }
      ...
      // Non-error cases.
        ...
        if (image != null) {
          BaoLog.d(TAG, "Got an image response.")
          resultImageToShow = image
        }
        if (webview != null) {
          BaoLog.d(TAG, "Got an webview response.")
          val webviewUrl =
            skillManagerViewModel.getJsSkillWebviewUrl(
              skillName = skillName,
              url = webview.url ?: "",
            )
          BaoLog.d(TAG, "Webview url: $webviewUrl")
          resultWebviewToShow = webview.copy(url = webviewUrl)
        }
        BaoLog.d(TAG, "Result: ${resultJson.result}")
  ...
  fun runIntent(
    ...
    return runBlocking(Dispatchers.Default) {
      if (IntentAction.from(intent) == null) {
        BaoLog.w(TAG, "Intent not found: '$intent'")
        return@runBlocking guardMissingEntityWithSkillFallback(name = intent, type = "Intent")
      }
      BaoLog.d(TAG, "Run intent. Intent: '$intent', parameters: '$parameters'")
      _actionChannel.send(
        SkillProgressAgentAction(
