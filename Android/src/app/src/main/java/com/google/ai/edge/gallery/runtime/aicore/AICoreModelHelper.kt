...
object AICoreModelHelper : LlmModelHelper {
  ...
  override fun initialize(
    ...
    if (coroutineScope == null) {
      BaoLog.e(TAG, "CoroutineScope is required for AICoreModelHelper")
    ...
    coroutineScope.launch {
      runCatching {
            ...
            generativeModel.download().collect { downloadStatus ->
              when (downloadStatus) {
                is DownloadStatus.DownloadStarted -> {
                  // Progress events must NOT fire the terminal init `onDone` callback (that flips the
                  // model to a spurious ERROR while still downloading). Log progress instead.
                  BaoLog.d(TAG, "AICore download started (${downloadStatus.bytesToDownload} bytes)")
                }
                is DownloadStatus.DownloadProgress -> {
                  BaoLog.d(TAG, "AICore download progress (${downloadStatus.totalBytesDownloaded} bytes)")
                }
        ...
        BaoLog.e(TAG, "Initialization failed", e)
  ...
  fun downloadModel(
    ...
    coroutineScope.launch {
      runCatching {
        ...
        BaoLog.e(TAG, "Download failed", e)
  ...
  suspend fun isModelDownloaded(model: Model): Boolean {
    ...
    return runCatching {
      ...
      BaoLog.e(TAG, "Failed to check AICore model status", e)
      false
    }
  }
  ...
  override fun runInference(
    ...
    if (coroutineScope == null) {
      BaoLog.e(TAG, "CoroutineScope is required for AICoreModelHelper inference")
