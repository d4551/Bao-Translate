 ...
 *   the top bar.
 */
open class CustomTaskData(
  open val modelManagerViewModel: ModelManagerViewModel,
...
data class CustomTaskDataForBuiltinTask(
  override val modelManagerViewModel: ModelManagerViewModel,
  val onNavUp: () -> Unit,
  // The initial query to be sent to the model when the screen is first loaded.
  val initialQuery: String? = null,
) : CustomTaskData(modelManagerViewModel = modelManagerViewModel)
