...
constructor(
  ...
  fun runBenchmark(
    ...
    viewModelScope.launch(Dispatchers.Default) {
      ...
      BaoLog.d(TAG, "Running benchmark: ${parts.joinToString("\n")}")
      ...
      // Isolated cache dir scoped to this benchmark run.
      ...
      if (!benchmarkCacheDir.mkdirs()) {
        BaoLog.e(TAG, "Failed to create benchmark cache directory: ${benchmarkCacheDir.absolutePath}")
        cacheDirPath = appContext.cacheDir.absolutePath
        needCleanUpCacheDir = false
      }
      BaoLog.d(TAG, "Using benchmark cache dir: $cacheDirPath")
      val backend: Backend =
      ...
      runCatching {
        ...
        BaoLog.e(TAG, "Benchmark failed", e)
      ...
      if (needCleanUpCacheDir) {
        benchmarkCacheDir.deleteRecursively()
        BaoLog.d(TAG, "Cleaned up benchmark cache dir: ${benchmarkCacheDir.absolutePath}")
      }
      ...
      val basicInfo =
        LlmBenchmarkBasicInfo.newBuilder()
          ...
          .setAppVersion(ProjectConfig.versionName)
          .build()
      val stats =
  ...
  fun addBenchmarkResult(result: BenchmarkResult): String {
    ...
    // Save to storage.
    viewModelScope.launch { dataStoreRepository.addBenchmarkResult(result) }
  ...
  fun deleteBenchmarkResult(id: String) {
    ...
    if (index != -1) {
      ...
      BaoLog.w(TAG, "Benchmark result with id $id not found.")
    }
  }
